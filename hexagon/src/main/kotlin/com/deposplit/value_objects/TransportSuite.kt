 ...
 * JSON-facing, since it never rides the wire except as the leading byte of a ciphertext blob:
 * the ciphertext wire format is `suiteTag(1) || nonce(12) || ciphertext+tag`. Needs no persistent
 * state or trust mechanism — each deposit and retrieval leg is re-derived fresh, so a device just
 * always encrypts with its current preferred suite and a decrypting device dispatches on the tag
 * it reads.
 */
